...
async function boot(){try{data=await loadData();$('#last-updated').textContent=data.generated_at||'—';$('#data-status').textContent=data.status==='live'?'LIVE DATA':data.status==='partial'?'LIVE / FALLBACK':'LATEST AVAILABLE';$('#data-status').classList.add('live');renderAll();bind()}catch(err){console.error(err);$('#data-status').textContent='DATA ERROR';document.querySelector('main').insertAdjacentHTML('afterbegin','<div class="error-banner">The latest liquidity dataset could not be loaded. The interface has stopped rather than presenting incomplete values as current.</div>')}}
boot();
